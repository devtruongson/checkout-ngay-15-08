...
class BookView {
    constructor() {
        ...
        querySelector(".cancel").addEventListener(
            "click",
        ...
        querySelector(".ascending").addEventListener(
            "click",
            this.handleAscendingClick
        );
        querySelector(".descending").addEventListener(
            "click",
            this.handleDescendingClick
        );
        this.overlay.addEventListener(
            "click",
            this.hideValidationForm.bind(this)
        );
    ...
    init = () => {
        ...
        this.showBooks();
        this.setupSearch();
        querySelector(".btn-confirm").addEventListener(
            "click",
            this.handleConfirmDelete
        );
        querySelector(".btn-cancel").addEventListener(
            "click",
    ...
    handleSaveButtonClick = async (event) => {
        ...
        if (validateForm(this.validationForm)) {
            ...
            if (!InputImageUpload.files[0]) {
                alert("Bạn vui lòng tải ảnh lên!");
                return;
            }

            const API_KEY = "ae6e48ff162fc31924cda2ebbd8d7e80";
    ...
    displayAllBooks = (books) => {
        ...
        <li class="book" data-book-index="${startIndex + index}">
          ...
          <img src="${
              bookInfo.image
          }" alt="detail" class="detail" style="margin-top: 10px;height: 100px; object-fit:cover;" data-book-index="${
                startIndex + index
            }">
        </a>
